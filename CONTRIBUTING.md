...
* You [license](https://github.com/shaynevanasperen/code/blob/master/LICENSE) the contribution under the terms applied to the rest of the _Code_ project.
* You agree to follow the [code of conduct](https://github.com/shaynevanasperen/code/blob/master/CODEOFCONDUCT.md).
...
The packages in _Code_ should have no explicit dependencies on compiled libraries. They may have implicit dependencies,
meaning that you could have a package which extends something in a compiled library, but it doesn't appear in the
`<dependencies>` element of the `.nuspec` file. This is because consumers of such a package would already necessarily
be depending on that library, and since the code would be compiled with the consumer code we allow the consumer to
decide which version they want.

### Naming

Packages should be named according to the type and namespace they are extending. If a package is an extension method,
the package name should start with `Code.Extensions` and then be followed by the fully qualified name of the type
being extended, ending with the name of extension method. In the case that the namespace of the type being extended starts
with `System`, then that part of the namespace is omitted from the package name. If the extension method extends a
generic type without type constraints, then the package name should start with `Code.Extensions.Generic` followed
by the name of the extension method. If the extension method extends a generic type with type constraints, then the
package name should start with `Code.Extensions.Generic` followed by the name of the most prominent type constraint
and then followed by the name of the extension method.

If the package is a standalone class, it's name should start with `Code` and then be followed by the namespace of the
type it works with or the namespace of the interface(s) it implements, ending with the name of the class. In the case
that the namespace starts with `System`, then that part of the namespace is omitted from the package name.

Packages should be organised in the `Code` project in a folder hierarchy that matches the name of the package, with
each package consisting of a single `.cs` source code file accompanied by a `.nuspec` file.

By following these naming rules we can make it easier for consumers to know what the package does and if applicable,
 ...
 * Through GitHub, or through the [Gitter chat](https://gitter.im/code-packages) (preferred),
   you talk about a feature you would like to see (or a bug), and why it should be in _Code_.
 ...
 * You create, or update, a fork of `shaynevanasperen/code` under your GitHub account.
 ...
 * Once you feel it is ready, submit the pull request to the `shaynevanasperen/code` repository against the `master` branch
 ...
 * `git fetch upstream` (upstream would be the mainstream repo or `shaynevanasperen/code` in this case)
 * `git checkout master`
 * `git rebase upstream/master`
 * `git checkout your-branch`
 * `git rebase master`
 * Fix any merge conflicts
 * `git push origin your-branch` (origin would be your GitHub repo or `your-github-username/code` in this case).
...
with permission and was edited to follow _Code_'s conventions and processes.
