...
{
	/// <summary>
	/// Establishes whether connectivity can be made to the given <see cref="DnsEndPoint"/>.
		...
		SocketType socketType = SocketType.Stream, ProtocolType protocolType = ProtocolType.Tcp)
	{
		...
		try
		{
			...
			if (Socket.ConnectAsync(socketType, protocolType, socketAsyncEventArgs))
			{
				...
				await using (cancellationTokenSource.Token.Register(state =>
							 {
								 if (state != null)
									 Socket.CancelConnectAsync((SocketAsyncEventArgs)state);
		...
		{
			throw new($"Connection attempt to {name} host '{endPoint.Host}:{endPoint.Port}' timed out after {timeout.Seconds} seconds.", e);
		}
		catch (System.Exception e)
		{
			throw new($"Connection attempt to {name} host '{endPoint.Host}:{endPoint.Port}' failed.", e);
		}
	}
