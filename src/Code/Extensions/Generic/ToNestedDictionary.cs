...
namespace Code.Extensions.Generic
{
	static partial class DictionaryExtension
	{
		...
		private static void AppendToPath(this string segment, char keyDelimiter, ref string path) =>
			path = path.Length == 0
				? segment
				: string.Join(keyDelimiter.ToString(CultureInfo.InvariantCulture), path, segment);

		private static object GetChild(this IEnumerable<KeyValuePair<string, string>> config, char keyDelimiter, IComparer<string> keyComparer, string path, int depth)
		{
			var prefix = depth > 0
				? path + keyDelimiter
				: path;
			...
			return siblings.Where((sibling, i) => sibling != i.ToString(CultureInfo.InvariantCulture)).Any()
				...
				: (object)new object[siblings.Length];
		}
	}
}
