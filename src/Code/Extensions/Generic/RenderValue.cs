...
namespace Code.Extensions.Generic
{
	static partial class KeyValuePairExtension
	{
		...
		private static DictionaryValueTemplate Parse(this string valueTemplate, IDictionary<string, string> data, IReadOnlyCollection<string> path, StringComparison keyComparisonType) =>
			new DictionaryValueTemplate(valueTemplate, valueTemplate.Tokenize(data, path, keyComparisonType));

		private static IEnumerable<object> Tokenize(this string valueTemplate, IDictionary<string, string> data, IReadOnlyCollection<string> path, StringComparison keyComparisonType)
		{
		...
		private static bool IsValidKey(this string text, IEnumerable<string> validKeys, StringComparison keyComparisonType) =>
			validKeys.Any(x => string.Compare(text, x, keyComparisonType) == 0);

		private static TextToken ParseTextToken(this string valueTemplate, int startAt, out int next)
		{
