...
static class Command
{
		...
		Func<string, bool>? hasError = null)
	{
		    ...
		    error.WaitHandle.WaitOne(milliseconds))
		{
			...
			{
				throw new($"{command} exited with code {process.ExitCode}.{Environment.NewLine}{output.Text}{Environment.NewLine}{error.Text}");
			}
		}
		else
		{
			throw new($"{command} timed out.");
		}
	}

	static Process StartProcess(string command, string arguments, CommandType commandType, ILogger logger,
